...
AlphaSynth.Main.AlphaSynthApi = function (asRoot, swfObjectRoot){
    ...
    if (supportsWebAudio){
        AlphaSynth.Util.Logger.Debug("Will use webworkers for synthesizing and web audio api for playback");
...
AlphaSynth.Main.AlphaSynthWebAudioPlayer = function (){
    this._context = null;
    this._buffer = null;
    this._source = null;
...
AlphaSynth.Main.AlphaSynthWebAudioPlayer.prototype = {
    Play: function (){
        this.RequestBuffers();
        this._finished = false;
        if (this._paused){
    ...
    RequestBuffers: function (){
        ...
        // we request one half
        var count = 20480;
        if (this._circularBuffer.get_Count() < count && this.RequestBuffer != null){
            for (var i = 0; i < 5; i++){
                this.RequestBuffer();
            }
        }
    ...
    add_RequestBuffer: function (value){
        this.RequestBuffer = $CombineDelegates(this.RequestBuffer, value);
    },
    remove_RequestBuffer: function (value){
        this.RequestBuffer = $RemoveDelegate(this.RequestBuffer, value);
    },
...
$StaticConstructor(function (){
    AlphaSynth.Main.AlphaSynthWebAudioPlayer.BufferSize = 4096;
    AlphaSynth.Main.AlphaSynthWebAudioPlayer.Latency = 46;
    AlphaSynth.Main.AlphaSynthWebAudioPlayer.BufferCount = 10;
});
AlphaSynth.Main.AlphaSynthWebWorker = function (main){
    ...
    this._player.AddEventListener(this);
    this.OnReady();
};
...
AlphaSynth.Main.AlphaSynthWebWorkerApi = function (asRoot){
    ...
    this._events = null;
    this._asRoot = asRoot;
    this._player = new AlphaSynth.Main.AlphaSynthWebAudioPlayer();
    this._player.add_PositionChanged($CreateDelegate(this, this.PlayerPositionChanged));
    this._player.add_RequestBuffer($CreateDelegate(this, this.PlayerSampleRequest));
    ...
    HandleWorkerMessage: function (e){
        ...
        switch (cmd){
            ...
            case "finished":
                ...
                break;
            case "soundFontLoad":
                this.TriggerEvent(cmd, [data["loaded"], data["full"]]);
                break;
...
AlphaSynth.Player.SynthPlayerEventDispatcher = function (){
    this._listeners = null;
    this._listeners = [];
};
AlphaSynth.Player.SynthPlayerEventDispatcher.prototype = {
    OnPositionChanged: function (currentTime, endTime, currentTick, endTick){
...
AlphaSynth.Player.SynthPlayer = function (){
    ...
    this._events = new AlphaSynth.Player.SynthPlayerEventDispatcher();
    this.State = AlphaSynth.Player.SynthPlayerState.Stopped;
    this.FirePlayerStateChanged();
    AlphaSynth.Util.Logger.Debug("Opening output");
    this._output = AlphaSynth.Platform.Platform.CreateOutput();
    this._output.add_Finished($CreateAnonymousDelegate(this, function (){
        // stop everything
        this.Stop();
        AlphaSynth.Util.Logger.Debug("Finished playback");
        this._events.OnFinished();
    }));
    this._output.add_SampleRequest($CreateAnonymousDelegate(this, function (){
        // synthesize buffer
    ...
    this._output.add_PositionChanged($CreateAnonymousDelegate(this, function (pos){
        // log position
        this.FirePositionChanged(pos);
    }));
    AlphaSynth.Util.Logger.Debug("Creating synthesizer");
    this._synth = new AlphaSynth.Synthesis.Synthesizer(44100, 2, 441, 3, 100);
    this._sequencer = new AlphaSynth.Sequencer.MidiFileSequencer(this._synth);
    this._sequencer.AddFinishedListener($CreateDelegate(this._output, this._output.SequencerFinished));
};
AlphaSynth.Player.SynthPlayer.prototype = {
    ...
    LoadSoundFontUrl: function (url){
        ...
        loader.Progress = $CreateDelegate(this, this.OnSoundFontLoad);
        try{
            loader.Load();
        }
        catch(e){
    ...
    LoadMidiUrl: function (url){
        ...
        loader.Progress = $CreateDelegate(this, this.OnMidiLoad);
        try{
            loader.Load();
        }
        catch(e){
    ...
    FirePositionChanged: function (pos){
        var endTime = (((this._sequencer.EndTime / this._synth.SampleRate) | 0) * 1000);
        var currentTime = pos;
...
AlphaSynth.Synthesis.Synthesizer.prototype = {
    ...
    set_MasterVolume: function (value){
        this._masterVolume = AlphaSynth.Synthesis.SynthHelper.ClampF(value, 0, 3);
    },
